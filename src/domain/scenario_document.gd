...
func create_guided_mission_template(scenario_id:String,world:Dictionary,definitions:Array[Dictionary])->bool:
	...
	var first_center:Array=center.call(first);var second_center:Array=center.call(second);var route_direction:=Vector2(float(second_center[0])-float(first_center[0]),float(second_center[2])-float(first_center[2]));if route_direction.length_squared()<0.01:route_direction=Vector2(float(second_center[0])-hero_x,float(second_center[2])-hero_z);if route_direction.length_squared()<0.01:route_direction=Vector2.RIGHT;route_direction=route_direction.normalized()
	var goal_point:=[float(second_center[0])+route_direction.x*8.0,0.0,float(second_center[2])+route_direction.y*8.0];var branch_midpoint:=Vector2((float(first_center[0])+float(second_center[0]))*0.5,(float(first_center[2])+float(second_center[2]))*0.5);var reward_point:=[branch_midpoint.x-route_direction.y*6.0,0.0,branch_midpoint.y+route_direction.x*6.0]
