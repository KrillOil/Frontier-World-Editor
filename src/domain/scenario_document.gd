...
func create_guided_mission_template(scenario_id:String,world:Dictionary,definitions:Array[Dictionary],terrain=null)->bool:
	...
	if player.is_empty():missing_roles.append("player leader — place a unit whose Owner is player")
	if allies.is_empty():missing_roles.append("recruitable ally — place a unit whose Owner is ally")
	if hostiles.size()<2:missing_roles.append("hostile encounters — place %d more unit(s) whose Owner is hostile"%(2-hostiles.size()))
	if not missing_roles.is_empty():return _fail(["Guided Mission Template is missing: %s."%"; ".join(missing_roles)])
	...
	var hero_definition:Dictionary=by_definition[hero.definition_id];var tutorials:Array=[{"tutorial_id":"move_to_allies","text":"Move to the waiting allies.","control":"move","indicator":"both","acknowledgement":"input","region_id":"recruit_checkpoint"},{"tutorial_id":"select_squad","text":"The recruited allies joined your selected squad. Right-click the marked hostile camp.","control":"move","indicator":"both","acknowledgement":"input","region_id":"encounter_one_bounds"},{"tutorial_id":"attack_encounter","text":"Attack the awakened hostile group.","control":"attack","indicator":"both","acknowledgement":"input","region_id":"encounter_one_bounds"},{"tutorial_id":"claim_optional_reward","text":"Optional: take the side path to claim a permanent hero reward.","control":"move","indicator":"both","acknowledgement":"input","region_id":"reward_checkpoint"},{"tutorial_id":"approach_final_encounter","text":"Move to the marked final hostile camp.","control":"move","indicator":"both","acknowledgement":"input","region_id":"encounter_two_bounds"},{"tutorial_id":"attack_final_encounter","text":"Defeat the final hostile group, then continue to the mission goal.","control":"attack","indicator":"both","acknowledgement":"input","region_id":"encounter_two_bounds"},{"tutorial_id":"reach_mission_goal","text":"Move beyond the final encounter to the mission goal.","control":"move","indicator":"both","acknowledgement":"input","region_id":"mission_goal"}]
