...
func create_guided_mission_template(scenario_id:String,world:Dictionary,definitions:Array[Dictionary])->bool:
	var by_definition:={};for definition in definitions:by_definition[definition.definition_id]=definition
	var player:Array=[];var allies:Array=[];var hostiles:Array=[]
	...
	player.sort_custom(func(a,b):return int(not by_definition[a.definition_id].get("hero",false))<int(not by_definition[b.definition_id].get("hero",false)));hostiles.sort_custom(func(a,b):return a.instance_id<b.instance_id)
	...
	var hero:Dictionary=player[0];var recruit:Dictionary=allies[0];var first_point:Dictionary=first[0];var goal:Dictionary=second[-1]
	...
	var hero_definition:Dictionary=by_definition[hero.definition_id];var tutorials:Array=[{"tutorial_id":"select_player_unit","text":"Select your leader.","control":"select","indicator":"viewport","acknowledgement":"input","highlight":hero.instance_id},{"tutorial_id":"move_to_allies","text":"Move to the waiting allies.","control":"move","indicator":"both","acknowledgement":"input","region_id":"recruit_checkpoint"},{"tutorial_id":"select_squad","text":"Select the recruited squad together.","control":"group","indicator":"viewport","acknowledgement":"input","highlight":"recruitable_allies"},{"tutorial_id":"attack_encounter","text":"Attack the awakened hostile group.","control":"attack","indicator":"both","acknowledgement":"input","region_id":"encounter_one_bounds"}]
