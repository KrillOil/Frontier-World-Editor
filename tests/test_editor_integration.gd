...
func run() -> void:
	...
	var fake_godot:=ProjectSettings.globalize_path("user://Godot_v4.7.10-stable_linux.x86_64");var fake_file:=FileAccess.open(fake_godot,FileAccess.WRITE);fake_file.store_string("#!/bin/sh\nprintf '4.7.10.stable.official.fake\\n'\n");fake_file.close();var chmod_output:Array=[];_check(OS.execute("/bin/chmod",PackedStringArray(["+x",fake_godot]),chmod_output,true)==0,"Wrong-version Test Setup fixture is executable");editor.test_world_executable_field.text=fake_godot;editor.test_world_project_field.text=ProjectSettings.globalize_path("res://");editor.show_test_world_setup();editor._accept_test_world_setup();_check(editor.test_world_setup_dialog.visible and "Godot 4.7.1" in editor.status_label.text,"Test Setup rejects Godot 4.7.10 instead of accepting a substring version match");editor.test_world_setup_dialog.hide();DirAccess.remove_absolute(fake_godot)
	editor.test_world_executable_field.text=OS.get_executable_path();editor.test_world_project_field.text=ProjectSettings.globalize_path("res://");editor.show_test_world_setup();editor._accept_test_world_setup();_check(editor.test_world_setup_dialog.visible and "not Frontier" in editor.status_label.text,"Test Setup rejects a non-Frontier Godot project and keeps recovery open");editor.test_world_setup_dialog.hide()
	var frontier_project_path:=OS.get_environment("QA_FRONTIER_PROJECT_PATH")
	if not frontier_project_path.is_empty():editor.test_world_executable_field.text=OS.get_executable_path();editor.test_world_project_field.text=frontier_project_path;editor.show_test_world_setup();editor._accept_test_world_setup();_check(not editor.test_world_setup_dialog.visible and "validated" in editor.status_label.text.to_lower(),"Test Setup accepts the real Frontier/Game project with Godot 4.7.1")
	var had_project_default:=OS.has_environment("FRONTIER_PROJECT_PATH");var project_default:=OS.get_environment("FRONTIER_PROJECT_PATH");OS.set_environment("FRONTIER_PROJECT_PATH","/environment/default/Frontier/Game");editor.test_world_executable_field.text="/bin/true";editor.test_world_project_field.text="";editor.show_test_world_setup();var explicit_export_launch:Dictionary=editor.build_test_world_launch(editor.test_world_executable_field.text,editor.test_world_project_field.text,"/worlds/crimsdale","player_start");_check(editor.test_world_project_field.text.is_empty() and "--path" not in explicit_export_launch.arguments,"Explicitly clearing the project field overrides its environment default and preserves exported-build mode");editor.test_world_setup_dialog.hide();
	if had_project_default:OS.set_environment("FRONTIER_PROJECT_PATH",project_default)
	else:OS.unset_environment("FRONTIER_PROJECT_PATH")
