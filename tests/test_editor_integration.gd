...
func run() -> void:
	...
	var original_cinematic=editor.package.scenario.data.cinematics[0];editor.package.scenario.data.cinematics[0]="malformed";editor._validate_sequence_flow();editor.validation_list.select(0);editor._open_selected_validation_finding();_check(editor.cinematic_dialog.visible and editor.cinematic_list.is_selected(0),"A malformed cinematic member opens a type-safe exact-entry repair surface");editor.package.scenario.data.cinematics[0]=original_cinematic
	var original_sequence=editor.package.scenario.data.sequences[0];editor.package.scenario.data.sequences[0]="malformed";editor._validate_sequence_flow();editor.validation_list.select(0);editor._open_selected_validation_finding();_check(editor.sequence_dialog.visible and editor.sequence_list.get_selected_items().size()==1 and editor.sequence_list.get_item_metadata(editor.sequence_list.get_selected_items()[0]) is Dictionary,"A malformed sequence member opens a type-safe exact-entry repair surface");editor.package.scenario.data.sequences[0]=original_sequence
