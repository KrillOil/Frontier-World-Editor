...
func _init()->void:
	...
	if not scenario.data.is_empty():
		...
		var bypass_sequence:Dictionary=scenario._find(scenario.data.sequences,"sequence_id","hide_bypassed_optional_reward");var victory_index:int=scenario.data.sequences.find(scenario._find(scenario.data.sequences,"sequence_id","mission_victory"));var bypass_index:int=scenario.data.sequences.find(bypass_sequence)
		_check(bypass_sequence.enabled and bypass_sequence.one_shot and bypass_sequence.event=={"type":"unit_enters_region","group_id":"player_party","region_id":"mission_goal"} and bypass_sequence.conditions==[{"type":"group_alive","group_id":"encounter_two","value":false},{"type":"objective_is","objective_id":"claim_optional_reward","state":"active"}] and bypass_sequence.actions==[{"type":"set_objective","objective_id":"claim_optional_reward","state":"hidden"}] and bypass_index<victory_index,"Template hides an unclaimed optional reward at the goal before victory")
		_check(reward_sequence.actions.any(func(value):return value.type=="set_objective" and value.objective_id=="claim_optional_reward" and value.state=="completed") and reward_sequence.actions.any(func(value):return value.type=="grant_reward" and value.reward_id=="item_river_charm"),"Template preserves claimed optional reward completion and grant")
		var reward_handoff:Dictionary=scenario._find(scenario.data.sequences,"sequence_id","reward_final_approach_guidance")
		_check(not reward_sequence.actions.any(func(value):return value.get("type")=="show_tutorial") and reward_handoff.event=={"type":"sequence_completed","sequence_id":"claim_optional_reward"} and reward_handoff.conditions==[{"type":"sequence_has_run","sequence_id":"approach_final_encounter","value":false}] and reward_handoff.actions==[{"type":"show_tutorial","tutorial_id":"approach_final_encounter"}] and scenario._find(scenario.data.tutorials,"tutorial_id","approach_final_encounter").get("region_id")=="encounter_two_bounds","Reward completion shows marked final-encounter guidance only while the final approach has not run")
		...
		var start_sequence:Dictionary=scenario._find(scenario.data.sequences,"sequence_id","mission_start");var squad_tutorial:Dictionary=scenario._find(scenario.data.tutorials,"tutorial_id","select_squad");_check(start_sequence.actions.any(func(value):return value.get("tutorial_id")=="move_to_allies") and scenario._find(scenario.data.tutorials,"tutorial_id","select_player_unit").is_empty() and scenario._find(scenario.data.sequences,"sequence_id","begin_route").is_empty() and recruit_sequence.actions.any(func(value):return value.get("tutorial_id")=="select_squad") and squad_tutorial.control=="move" and squad_tutorial.region_id=="encounter_one_bounds" and "joined your selected squad" in squad_tutorial.text and "Right-click" in squad_tutorial.text and not recruit_sequence.actions.any(func(value):return value.get("tutorial_id")=="attack_encounter") and approach_sequence.actions.any(func(value):return value.get("tutorial_id")=="attack_encounter"),"Auto-selected movement, automatic recruitment, and attack guidance occur on separate actionable beats")
		var first_clear:Dictionary=scenario._find(scenario.data.sequences,"sequence_id","first_encounter_cleared");var approach_final:Dictionary=scenario._find(scenario.data.sequences,"sequence_id","approach_final_encounter")
		_check(not first_clear.actions.any(func(value):return value.get("type")=="set_encounter" and value.get("group_id")=="encounter_two") and approach_final.actions.any(func(value):return value.get("type")=="set_encounter" and value.get("group_id")=="encounter_two" and value.get("state")=="active"),"Final hostiles remain dormant through the optional branch and activate only after the authored final approach")
	...
	var insufficient=ScenarioScript.new();_check(not insufficient.create_guided_mission_template("cannot_build",{"objects":[]},[]) and "player leader" in insufficient.errors[0] and "recruitable ally" in insufficient.errors[0] and "2 more unit(s)" in insufficient.errors[0],"Template names every missing placed role and the hostile-unit shortfall")
	var definition_owners:={};for definition in package.definitions:definition_owners[definition.definition_id]=definition.get("owner","")
	var one_hostile_world:Dictionary=package.world.duplicate(true);one_hostile_world.objects=[];var hostile_count:=0
	for object in package.world.objects:
		if definition_owners.get(object.definition_id)=="hostile":
			hostile_count+=1
			if hostile_count>1:continue
		one_hostile_world.objects.append(object.duplicate(true))
	var one_hostile=ScenarioScript.new();_check(not one_hostile.create_guided_mission_template("needs_hostile",one_hostile_world,package.definitions) and "place 1 more unit(s)" in one_hostile.errors[0],"Template reports the exact remaining hostile role count")
