...
func _init()->void:
	...
	if not scenario.data.is_empty():
		...
		var begin_sequence:Dictionary=scenario._find(scenario.data.sequences,"sequence_id","begin_route");_check(begin_sequence.actions.any(func(value):return value.get("tutorial_id")=="move_to_allies") and recruit_sequence.actions.any(func(value):return value.get("tutorial_id")=="select_squad") and not recruit_sequence.actions.any(func(value):return value.get("tutorial_id")=="attack_encounter") and approach_sequence.actions.any(func(value):return value.get("tutorial_id")=="attack_encounter"),"Move, squad selection, and attack guidance occur on separate deliberate beats")
