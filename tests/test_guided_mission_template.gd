...
func _init()->void:
	...
	if not scenario.data.is_empty():
		...
		_check(reward_sequence.actions[-1].get("tutorial_id")=="approach_final_encounter" and scenario._find(scenario.data.tutorials,"tutorial_id","approach_final_encounter").get("region_id")=="encounter_two_bounds","Claiming the optional reward replaces its cue with marked final-encounter approach guidance")
