...
3. Confirm the world contains at least one player unit, one allied unit, and two hostile units. Use **Object Editor** to inspect ownership; use the palette and viewport to place any missing role. Also confirm **Object Editor** contains at least one item with Item Kind `permanent_stat`; the template uses it for the optional hero reward.
...
6. Confirm the status says the template was created. If it reports missing roles, place the named unit roles. If it reports a missing permanent-stat item, create one in **Object Editor**, set its effect stat and amount, save it, and try again.
...
8. Review the generated regions and the `guided_route` path. Select a region to edit its coordinates, then choose **Update Selected Region**. Put the recruit checkpoint on the allies, each encounter boundary around its hostile group, the optional reward checkpoint on a reachable side path, and the mission goal beyond the final encounter.
...
12. Open **Sequences** and inspect `claim_optional_reward`. Its actions must complete the optional step and objective, then grant the permanent-stat item to `player_party`. Choose **Validate Flow** and confirm the generated flow remains: start → recruit → approach and clear the first encounter → optional reward branch → final encounter → goal and victory.
...
17. Defeat the first group and confirm the second group activates. Follow the optional side-path prompt and confirm the leader receives the permanent reward. If the leader has an authored ability, select the leader and press `1` during the final encounter.
